...
void escreveCantina(LLMesas*ll) {//vai escrever a cantina como diz no nome
	...
	while (ll->iterator != NULL) {
		int l = 0;
		SetConsoleTextAttribute(hConsoleM, 240);
		...
		SetConsoleTextAttribute(hConsoleM, 15);
...
void ordenaAlfabeticamenteUltNome(LLMesas*cantina, LLPessoas*fila, LLPessoas*acabados) {//ordena alfabeticamente todas as pessoas que passaram pelo sistema pelos seus últimos nomes
	...
	system("CLS");
	SetConsoleTextAttribute(hConsoleM, 9);
	cout << "\t\t\t\t\t Cantina EDA" << endl;
	SetConsoleTextAttribute(hConsoleM, 224);
...
void mudaNome(LLMesas*cantina, LLPessoas*fila) {//função que permite a alteração do primeiro, último ou ambos os nomes de um dado indivíduo, identificado pelo seu número
	...
	bool restarta = true;
	cout << "Escreva o número de aluno / funcionário do indivíduo ao qual deseja alterar o nome." << endl << endl;
	cout << "**** Comando: ";
	cin >> n;
	cantina->iterator = cantina->primeira;
	fila->iterator = fila->primeira;
	...
	if (háPessoaFila || háPessoaCantina) {
		while (restarta) {
			cout << endl << "Deseja alterar" << endl << "\t1. O primeiro nome;" << endl << "\t2. O último nome;" << endl << "\t3. Ambos os nomes do indivíduo." << endl << endl;
			cout << "**** Comando: ";
			cin >> PriOuUltOuAmbos;
			cin.ignore(1000, '\n');
			...
			case 1:
				if (háPessoaCantina) {
					SetConsoleTextAttribute(hConsoleM, 12);
					cout << endl << "Escreva o novo primeiro nome do indivíduo." << endl << endl << "Novo primeiro nome: ";
					SetConsoleTextAttribute(hConsoleM, 15);
			...
			case 2:
				if (háPessoaCantina) {
					SetConsoleTextAttribute(hConsoleM, 12);
					cout << endl << "Escreva o novo último nome do indivíduo." << endl << endl << "Novo último nome: ";
					SetConsoleTextAttribute(hConsoleM, 15);
...
void editaDuração(LLMesas* cantina, LLPessoas*fila) {//permite a alteração da duração de um grupo / departamento, identificado pelo número de grupo / departamento
	...
	cout << "Escreva o número do grupo / departamento ao qual deseja alterar a duração de refeição." << endl << endl << "**** Comando: ";
	cin >> num;
	while (tentaOutraVez) {
		cout << endl << "Escreva a nova duração de refeição dos elementos do grupo / departamento." << endl << endl << "**** Comando: ";
		cin >> novaDuração;
		if (novaDuração <= 0) {
			SetConsoleTextAttribute(hConsoleM, 4);
			cout << "A nova duração tem que ser maior que 0!" << endl;
			SetConsoleTextAttribute(hConsoleM, 15);
		}
		else {
			tentaOutraVez = false;
...
void SelectionSortMesasNumSentados(LLMesas::mesa**cantina, int tam) {//ordena a cantina pelo número de pessoas sentadas nas mesas, ordem decrescente
	...
	cout << "\t\t\t\t\t Cantina EDA" << endl;
	SetConsoleTextAttribute(hConsoleM, 224);
	cout << "Aqui encontram-se as mesas, ordenadas pelo número de pessoas sentadas, do menor número para o maior:" << endl << endl;
...
void apresentaIndividuo(LLMesas*cantina, LLPessoas*fila) {//apresenta todos os detalhes de um indivíduo identificado pelo seu número pelo utilizador
	int n;
	cout << "Escreva o número de aluno / funcionário do indivíduo do qual deseja conhecer todos os dados." << endl << endl;
	cout << "**** Comando: ";
	cin >> n;
	cantina->iterator = cantina->primeira;
	fila->iterator = fila->primeira;
		...
		while (cantina->iterator->sentados->iterator != NULL) {
			if (cantina->iterator->sentados->iterator->membro_aluno.num > 0) {
				if (cantina->iterator->sentados->iterator->membro_aluno.num == n) {
					...
					cout << "\t\t\t\t\t Cantina EDA" << endl;
					SetConsoleTextAttribute(hConsoleM, 224);
					cout << "Aqui está o indivíduo que pediu:" << endl << endl;
					SetConsoleTextAttribute(hConsoleM, 15);
	...
	SetConsoleTextAttribute(hConsoleM, 4);
	cout << "Esse indivíduo não está na cantina ou na fila." << endl;
	SetConsoleTextAttribute(hConsoleM, 15);
}
...
void apresentaCursoOuDep(LLMesas * cantina, LLPessoas * fila, string *CURSOS) {//apresenta os indivíduos de um dado curso ou departamento
	...
	while (restarta) {
		...
		{
		case 1:
			SetConsoleTextAttribute(hConsoleM, 95);
			cout << endl << "Cursos possíveis:" << endl;
			while (i < 19) {
			...
			i = 0;
			SetConsoleTextAttribute(hConsoleM, 15);
			cout << endl << "Indique o curso que pretende." << endl << endl;
			cout << "**** Comando: ";
			cin >> cursoAProcurar;
	...
	if (oCurso != "skip") {
		...
		cout << "\t\t\t\t\t Cantina EDA" << endl;
		SetConsoleTextAttribute(hConsoleM, 224);
		cout << "Estes são os alunos que estão a estudar " << oCurso << " que estão na cantina ou na fila:" << endl << endl;
		SetConsoleTextAttribute(hConsoleM, 15);
		pos = 0;
		while (pos < tam) {
		...
		cout << "\t\t\t\t\t Cantina EDA" << endl;
		SetConsoleTextAttribute(hConsoleM, 224);
		cout << "Estes são os funcionários do departamento " << numDepartamento << " que estão na cantina ou na fila:" << endl << endl;
		SetConsoleTextAttribute(hConsoleM, 15);
		while (pos < tam) {
...
void mudaCurso(LLMesas*cantina, LLPessoas*fila, string* CURSOS) {//permite a alteração de curso de um dado grupo de alunos
	bool háGrupo = false;
	int numGrupo;
	cout << "Pretende mudar o curso de qual grupo de alunos?" << endl << endl;
	cout << "**** Comando: ";
	cin >> numGrupo;
	cin.ignore(1000, '\n');
	cantina->iterator = cantina->primeira;
	...
	SetConsoleTextAttribute(hConsoleM, 15);
	while (restarta) {
		cout << endl << "Indique o novo curso que pretenda que o grupo tenha." << endl << endl;
		cout << "**** Comando: ";
		cin >> cursoAProcurar;
		cin.ignore(1000, '\n');
		if (cursoAProcurar <= 19) {
...
void PobreRicoEMedia(LLMesas*cantina, LLPessoas*fila) {//apresenta a pessoa mais pobre, mais rica e o plafond médio
	...
	SetConsoleTextAttribute(hConsoleM, 9);
	cout << "\t\t\t\t\t Cantina EDA" << endl;
	SetConsoleTextAttribute(hConsoleM, 47);
	...
	cout << endl << endl;
	SetConsoleTextAttribute(hConsoleM, 143);
	...
	cout << endl << endl;
	SetConsoleTextAttribute(hConsoleM, 111);
...
void lotaria(LLMesas*cantina, LLPessoas*fila, bool *háVencedor, unsigned int*premio, int*aumenta) {//uma sortuda pessoa ganha o prémio, que é adicionado ao seu plafond. Se ninguém ganha o prémio duplica mas a probabilidade de alguém ganhar também diminui
	cantina->iterator = cantina->primeira;
	int*naCantina = new int(0);
	while (cantina->iterator != NULL) {
		*naCantina += cantina->iterator->numSentados;
