...
#include<fstream>
#include <Windows.h>
HANDLE hConsoleP = GetStdHandle(STD_OUTPUT_HANDLE);
using namespace std;
...
void escreveFila(LLPessoas*fila) {//como o nome indica escreve um vetor de pessoa*
	SetConsoleTextAttribute(hConsoleP, 240);
	cout << endl << "FILA DE ESPERA:" << endl;
	SetConsoleTextAttribute(hConsoleP, 15);
	int i = 1;
	fila->iterator = fila->primeira;
	...
	while (fila->iterator != NULL) {
		if (fila->iterator->membro_aluno.num > 0) {
			if (fila->iterator->plafond < r->atual->custo) {
				SetConsoleTextAttribute(hConsoleP, 79);
				cout << "***** ATENÇÃO *****" << endl;
				SetConsoleTextAttribute(hConsoleP, 15);
				cout << "O aluno com o número " << fila->iterator->membro_aluno.num << " não possui plafond suficiente para iniciar a refeição." << endl;
			...
			if (fila->iterator->plafond < r->atual->custo) {
				...
				cout << "***** ATENÇÃO *****" << endl;
				SetConsoleTextAttribute(hConsoleP, 15); 
				cout << "O elemento de staff com o número " << fila->iterator->membro_staff.numFuncionario << " não possui plafond suficiente para iniciar a refeição." << endl;
	...
	if (háAluno || háStaff) {
		while (again) {
			...
			case 2:
				...
				break;
			default:
				SetConsoleTextAttribute(hConsoleP, 4);
	...
	while (ll->iterator != NULL) {
		if (ll->iterator->membro_aluno.num > 0) {
			if (ll->iterator->membro_aluno.num == n) {
				cout << "Plafond inicial: " << ll->iterator->plafond << " EUR" << endl << endl;
